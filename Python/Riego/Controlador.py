...
class Controlador:
    ...
    def activar_senal(self, estado, indice1, indice2):
        print("SE PRENDERA EL LED")
        if estado:
            mensaje = self.convertidor.generar_mensaje(CONTROL, MOD_BANDERA, [indice1, 1])
        else:
            mensaje = self.convertidor.generar_mensaje(CONTROL, MOD_BANDERA, [indice2, 1])
        print(f"El mensaje a enviar es: {mensaje}  ", ' '.join('{0:02X}'.format(e) for e in mensaje))
        valor = self.puerto_serie.enviar_mensaje(mensaje)
